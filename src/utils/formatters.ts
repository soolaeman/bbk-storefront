...
export function generateWhatsAppProductLink(product: Product, customAction?: 'video' | 'visit' | 'shipping' | 'quote'): string {
  let message = '';
  
  if (product.price === null || customAction === 'quote') {
    message = `Halo Tim BBKitchen, saya tertarik dan ingin menanyakan penawaran harga & ketersediaan untuk unit:\n\n` +
      `📦 *${product.name}*\n` +
      `🔖 SKU/ID: ${product.sku}\n` +
      `📍 Lokasi Unit: ${product.location}\n` +
      `⚙️ Kondisi: ${product.condition}\n\n` +
    ...
    message = `Halo Tim BBKitchen, saya ingin meminta *Video Tes Fungsi / Detail Fisik* untuk unit:\n\n` +
      `📦 *${product.name}*\n` +
      `🔖 SKU/ID: ${product.sku}\n` +
    ...
    message = `Halo Tim BBKitchen, saya berminat cek fisik langsung ke lokasi untuk unit:\n\n` +
      `📦 *${product.name}*\n` +
      `🔖 SKU/ID: ${product.sku}\n` +
      `📍 Lokasi: ${product.location}\n\n` +
      `Kira-kira kapan jadwal yang memungkinkan untuk survei/cek fisik unit ini? Terima kasih.`;
  } else if (customAction === 'shipping') {
    message = `Halo Tim BBKitchen, saya ingin konsultasi ongkos kirim dan pengantaran untuk unit:\n\n` +
      `📦 *${product.name}*\n` +
      `🔖 SKU/ID: ${product.sku}\n` +
    ...
    // Default inquiry
    message = `Halo Tim BBKitchen, saya tertarik dengan unit katalog:\n\n` +
      `📦 *${product.name}*\n` +
      `🔖 SKU: ${product.sku}\n` +
      `💰 Harga: ${product.price ? formatRupiah(product.price) : 'Tanyakan Harga'}\n` +
      `📍 Lokasi: ${product.location}\n` +
      `⚙️ Kondisi: ${product.condition} (${product.conditionRating}/10)\n\n` +
...
export function generateWhatsAppConsultationLink(topic?: string): string {
  const defaultText = `Halo Tim BBKitchen, saya ingin konsultasi kebutuhan peralatan dapur komersial untuk usaha saya (Restoran/Cafe/Katering/Bakery/MBG Kitchen).\n\nBisa dibantu rekomendasi alat yang sesuai menu dan estimasi budget modal kami? Terima kasih.`;
  const text = topic ? `Halo Tim BBKitchen, saya ingin bertanya perihal: ${topic}` : defaultText;
...
export function generateWhatsAppSourcingLink(itemDetails: {
  ...
  const message = `Halo Tim Sourcing BBKitchen, saya ingin menitip pencarian unit peralatan dapur komersial:\n\n` +
