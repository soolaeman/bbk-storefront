...
const MBG_WHATSAPP_LINK = generateWhatsAppCustomLink(
...
export const Header: React.FC<HeaderProps> = ({ simple = false }) => {
  ...
  const openMbgWhatsApp = () => {
    window.open(MBG_WHATSAPP_LINK, '_blank', 'noopener,noreferrer');
    setMobileMenuOpen(false);
  };

  const handleProductionClick = () => {
    window.open(PRODUCTION_WHATSAPP_LINK, '_blank', 'noopener,noreferrer');
    setMobileMenuOpen(false);
  };
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-6">
        <div className="flex min-h-[68px] items-center gap-3 py-3 md:gap-5">
          ...
          <div className="order-2 ml-auto flex shrink-0 items-center gap-2 md:order-3">
            {!simple && <>
              <button type="button" onClick={openMbgWhatsApp} className="hidden items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50 sm:inline-flex">
                <span>Dapur MBG</span>
              </button>
