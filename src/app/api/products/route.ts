...
function matchesMetaFilter(
  ...
  if (!expected || normalizeText(expected) === 'semua' || normalizeText(expected).startsWith('semua ')) {
    return true;
  }
...
async function fetchAllProductsForMetaFiltering(
  params: URLSearchParams,
  authorization: string,
): Promise<WooCommerceProductForMetaFilter[]> {
  ...
  const firstResponse = await fetch(`${WOOCOMMERCE_API_URL}/products?${firstParams.toString()}`, {
    headers: { Accept: 'application/json', Authorization: authorization },
    cache: 'no-store',
  });

  if (!firstResponse.ok) {
    throw new Error(`WooCommerce metadata product lookup gagal: ${firstResponse.status} ${firstResponse.statusText}`);
  }
  ...
  for (let page = 2; page <= totalPages; page += 1) {
    ...
    const response = await fetch(`${WOOCOMMERCE_API_URL}/products?${pageParams.toString()}`, {
      headers: { Accept: 'application/json', Authorization: authorization },
      cache: 'no-store',
    });

    if (!response.ok) {
      throw new Error(`WooCommerce metadata product page ${page} gagal: ${response.status} ${response.statusText}`);
    }
