...
function mapStatus(value: string): AvailabilityStatus {
  ...
  if (normalized === 'READY' || normalized === 'DP' || normalized === 'SOLD') {
    return normalized;
  }
...
function mapCondition(value: string): ProductCondition {
  const normalized = value.trim().toUpperCase();
  return (normalized === 'BARU' || normalized === 'BEKAS' ? normalized : '') as ProductCondition;
}
...
function mapProduct(product: WooCommerceProduct): Product {
  ...
  return {
    ...
    status: mapStatus(statusUnit),
    condition: mapCondition(kondisiUnit),
    conditionRating: 0,
    ...
    dateAdded:
      product.date_created || product.date_modified || new Date().toISOString(),
...
export async function getWooCommerceProductsResult(
  ...
  appendQueryParam(
    url,
    'status_unit',
    options?.statusFilter === 'READY_ONLY'
      ? 'READY'
      : options?.statusFilter === 'INCLUDE_SOLD'
        ? 'READY,DP,SOLD'
        : undefined,
  );
  appendQueryParam(
  ...
  appendQueryParam(
    ...
    'min_price',
    options?.minPrice ??
      (options?.minPriceNumber != null
        ? String(options.minPriceNumber)
        : undefined),
  );
  appendQueryParam(
    url,
    'max_price',
    options?.maxPrice ??
      (options?.maxPriceNumber != null
        ? String(options.maxPriceNumber)
        : undefined),
  );
