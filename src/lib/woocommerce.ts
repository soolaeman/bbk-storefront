...
interface WooCommerceAttribute { name: string; option?: string; options?: string[]; }
interface WooCommerceProduct {
  ...
  categories: WooCommerceCategory[]; attributes?: WooCommerceAttribute[]; stock_status: string;
  date_created?: string; date_modified?: string; meta_data?: WooCommerceMeta[];
}

export interface WooCommerceProductsQuery {
  perPage?: number; page?: number; search?: string; category?: string;
  stockStatus?: string; orderby?: 'date' | 'title' | 'price' | 'id'; order?: 'asc' | 'desc';
  sku?: string; featured?: boolean; minPrice?: string; maxPrice?: string;
  tag?: string; attribute?: string; attributeTerm?: string;
  condition?: string; location?: string; powerType?: string;
  statusFilter?: 'ALL' | 'READY_ONLY' | 'INCLUDE_SOLD';
  minPriceNumber?: number | null; maxPriceNumber?: number | null;
...
function normalizeKey(value: string): string { return value.toLowerCase().trim().replace(/[\s-]+/g, '_'); }
...
function getAttribute(product: WooCommerceProduct, keys: string[]): string {
  ...
  return attribute.options?.length ? attribute.options.join(', ').trim() : '';
}
...
function mapCondition(value: string): ProductCondition { return value.trim() as ProductCondition; }
...
function parsePrice(product: WooCommerceProduct): number | null {
  const raw = product.price || product.regular_price; if (!raw) return null;
...
function mapProduct(product: WooCommerceProduct): Product {
  ...
  return {
    id: String(product.id), sku: kodeUnit || product.sku || `BBK-${product.id}`, name: stripHtml(product.name || ''),
    category: mapCategory(product), brand: brand || 'Tidak tercantum', price: parsePrice(product), originalPriceEstimate: null,
    status: mapStatus(statusUnit, product.stock_status), condition: mapCondition(kondisiUnit), conditionRating: 0,
    location: lokasiUnit || 'Tidak tercantum', powerType: mapPowerType(product), powerWattage: powerWattage || undefined,
    dimensions: dimensions || undefined, material: material || undefined, summary, description, testedFunctions, images,
    dateAdded: product.date_created || product.date_modified || new Date().toISOString(), previousUsage: kondisiUnit || undefined,
    adminTelegramRef: linkTelegram || undefined, featured: false,
  };
}
function appendQueryParam(url: URL, key: string, value: string | number | boolean | undefined): void {
  if (value === undefined || value === null) return; const stringValue = String(value).trim(); if (stringValue) url.searchParams.set(key, stringValue);
}
...
export async function getWooCommerceProductsResult(options?: WooCommerceProductsQuery): Promise<WooCommerceProductsResult> {
  ...
  appendQueryParam(url, 'sku', options?.sku); appendQueryParam(url, 'featured', options?.featured);
  ...
  appendQueryParam(url, 'tag', options?.tag); appendQueryParam(url, 'attribute', options?.attribute); appendQueryParam(url, 'attribute_term', options?.attributeTerm);
