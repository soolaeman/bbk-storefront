...
export default function App() {
  ...
  useEffect(() => { let cancelled = false; const loadCatalogMetadata = async () => { setIsLoadingMetadata(true); try { const response = await fetch('/api/products?metadata=1', { headers: { Accept: 'application/json' }, cache: 'no-store' }); if (!response.ok) throw new Error(`Metadata endpoint gagal: ${response.status}`); const data = (await response.json()) as CatalogMetadata; if (!cancelled) setCatalogMetadata({ categories: Array.isArray(data.categories) ? data.categories : [], conditionOptions: Array.isArray(data.conditionOptions) ? data.conditionOptions : [], locationOptions: Array.isArray(data.locationOptions) ? data.locationOptions : [], totalProducts: typeof data.totalProducts === 'number' ? data.totalProducts : null }); } catch (error) { console.error('Failed to load WooCommerce catalog metadata:', error); } finally { if (!cancelled) setIsLoadingMetadata(false); } }; void loadCatalogMetadata(); return () => { cancelled = true; }; }, []);
  const loadProducts = async (page = 1) => { setIsLoadingProducts(true); setProductLoadError(null); try { const result = await getWooCommerceProductsResult({ perPage: PRODUCTS_PER_PAGE, page, search: filterState.searchQuery.trim() || undefined, category: filterState.category !== 'Semua' ? filterState.category : undefined, condition: filterState.condition !== 'Semua Kondisi' ? filterState.condition : undefined, location: filterState.location !== 'Semua Lokasi' ? filterState.location : undefined, powerType: filterState.powerType !== 'Semua Sumber Daya' ? filterState.powerType : undefined, statusFilter: filterState.statusFilter, minPriceNumber: filterState.minPrice, maxPriceNumber: filterState.maxPrice, sortBy: filterState.sortBy }); setProducts(result.products); setTotalResults(result.total); setCatalogPage(page); setCatalogPageInput(String(page)); setTotalPages(result.totalPages); setHasNextPage(result.totalPages !== null ? page < result.totalPages : result.products.length === PRODUCTS_PER_PAGE); } catch (error) { console.error('Failed to load WooCommerce products:', error); setProducts([]); setTotalResults(null); setTotalPages(null); setHasNextPage(false); setProductLoadError('Katalog unit sedang tidak dapat dimuat. Silakan coba lagi atau hubungi Tim BBKitchen.'); } finally { setIsLoadingProducts(false); } };
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans selection:bg-amber-400 selection:text-slate-950">
    ...
    <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8">
      ...
      <div className="mt-8"><KitchenConsultationBanner onRequestClick={() => setIsRequestModalOpen(true)} /></div>
    </main>
    <TrustSection />
    <TestimonialsSection />
    <LocationSection />
