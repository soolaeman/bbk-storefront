...
async function resolveWooCommerceCategoryId(categoryName: string, authorization: string): Promise<number | null> {
  ...
  const response = await fetch(buildWooCommerceUrl('products/categories', params), { headers: getWooCommerceHeaders(authorization), next: { revalidate: PRODUCT_CACHE_REVALIDATE_SECONDS } });
...
async function fetchWooCommerceCategories(authorization: string): Promise<WooCommerceMetadataCategory[]> {
  ...
  const categoriesResponse = await fetch(buildWooCommerceUrl('products/categories', params), { headers: getWooCommerceHeaders(authorization), next: { revalidate: 300 } });
  ...
  for (let page = 2; page <= totalPages; page += 1) {
    ...
    const response = await fetch(buildWooCommerceUrl('products/categories', params), { headers: getWooCommerceHeaders(authorization), next: { revalidate: 300 } });
...
async function fetchAllProductsForMetaFiltering(params: URLSearchParams, authorization: string): Promise<WooCommerceProductForMetaFilter[]> {
  ...
  const firstResponse = await fetch(buildWooCommerceUrl('products', firstParams), { headers: getWooCommerceHeaders(authorization), cache: 'no-store' });
  ...
  for (let page = 2; page <= totalPages; page += 1) { const pageParams = new URLSearchParams(firstParams); pageParams.set('page', String(page)); const response = await fetch(buildWooCommerceUrl('products', pageParams), { headers: getWooCommerceHeaders(authorization), cache: 'no-store' }); if (!response.ok) throw new Error(`WooCommerce metadata product page ${page} gagal: ${response.status} ${response.statusText}`); allProducts.push(...((await response.json()) as WooCommerceProductForMetaFilter[])); }
  return allProducts;
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(buildWooCommerceUrl('products', params), { headers: getWooCommerceHeaders(authorization), next: { revalidate: PRODUCT_CACHE_REVALIDATE_SECONDS } });
