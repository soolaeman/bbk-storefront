      ...
      <main>
        <section className="relative overflow-hidden bg-slate-950 text-white">
          ...
          <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-20 lg:py-24">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-sm sm:text-base leading-7 text-slate-300">Solusi peralatan dapur komersial untuk kebutuhan preparation, cooking, washing, storage, packing, dan exhaust pada operasional Dapur MBG/SPPG.</p>
        ...
        <section id="equipment" className="max-w-7xl mx-auto px-4 py-12 sm:py-16">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-2 text-2xl sm:text-3xl font-black tracking-tight">Peralatan Dapur MBG</h2>
            <p className="mt-3 text-sm leading-6 text-slate-600">BBKitchen menyediakan berbagai equipment untuk kebutuhan area kerja dapur MBG dan SPPG.</p>
          </div>
        ...
        <section className="bg-white border-y border-slate-200">
          <div className="max-w-7xl mx-auto px-4 py-12 sm:py-16 grid lg:grid-cols-[1.15fr_0.85fr] gap-8 items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Layout Dapur MBG / SPPG</p>
              <h2 className="mt-2 text-2xl sm:text-3xl font-black tracking-tight">Bagaimana layout Dapur MBG seharusnya disusun?</h2>
              <p className="mt-4 text-sm leading-6 text-slate-600">Alur kerja dapur dapat disusun dari penerimaan bahan, penyimpanan, persiapan, pengolahan, pemorsian/pengemasan, hingga loading agar setiap area memiliki fungsi yang jelas.</p>
              <div className="mt-5 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-600">
                {['Penerimaan', 'Penyimpanan', 'Persiapan', 'Pengolahan', 'Pemorsian / Packing', 'Loading'].map((item, index) => <span key={item} className="inline-flex items-center gap-2"><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5">{item}</span>{index < 5 && <ArrowRight className="w-3 h-3 text-slate-400" />}</span>)}
              </div>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-4 py-10 sm:py-12">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Peralatan SPPG Sesuai Kebutuhan Operasional</p>
                <h2 className="mt-1 text-lg sm:text-xl font-black">Peralatan SPPG Sesuai Kebutuhan Operasional</h2>
                <p className="mt-2 max-w-3xl text-xs sm:text-sm leading-5 text-slate-600">BGN menyediakan pedoman mengenai kebutuhan peralatan dapur SPPG. Komposisi equipment perlu disesuaikan dengan kapasitas dan kondisi dapur. Pedoman BGN menyebut kebutuhan alat disusun berdasarkan pengalaman dan evaluasi SPPG serta kapasitas layanan.</p>
              </div>
              <a href={BGN_JUKNIS_URL} target="_blank" rel="noopener noreferrer" className="shrink-0 inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs font-black text-slate-800 hover:bg-slate-100 transition-colors">Lihat Pedoman BGN <ArrowRight className="w-3.5 h-3.5" /></a>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-4 py-12 sm:py-16">
          <div className="rounded-3xl bg-slate-900 p-6 sm:p-8 lg:p-10 text-white grid lg:grid-cols-[1fr_auto] gap-8 items-center">
            <div>
              ...
              <h2 className="mt-2 text-2xl sm:text-3xl font-black tracking-tight">Paket Hemat Peralatan Dapur MBG</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">BBKitchen menyediakan opsi paket equipment untuk kebutuhan Dapur MBG. Detail equipment, jumlah, ukuran, dan kebutuhan instalasi tersedia dalam katalog.</p>
              <p className="mt-4 text-xs text-slate-400">Katalog mencantumkan harga Rp250 juta per 1 September 2025. Harga dapat berubah sebelum kontrak.</p>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-4 py-14 sm:py-20 text-center">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Konsultasi Dapur MBG</p>
          <h2 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight">Sedang menyiapkan Dapur MBG atau SPPG?</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">Konsultasikan kebutuhan equipment, kapasitas, dan layout dapur Anda dengan BBKitchen.</p>
